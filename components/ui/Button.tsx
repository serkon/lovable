  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive hover:cursor-pointer",
  {
    variants: {
      variant: {
        ...
        link: "text-primary underline-offset-4 hover:text-primary-400",
      },
      size: {
        default: "h-[var(--size-lg)] px-4 py-2 has-[>svg]:px-3",
        xs: "h-[var(--size-xs)] px-2 text-xs",
        sm: "h-[var(--size-sm)] rounded-md gap-1.5 px-3 has-[>svg]:px-2.5",
        md: "h-[var(--size-md)] rounded-md gap-1.5 px-3 has-[>svg]:px-2.5",
        lg: "h-[var(--size-lg)] rounded-md px-6 has-[>svg]:px-4 text-base",
        xl: "h-[var(--size-xl)] rounded-md px-8 text-base",
        icon: "size-[var(--size-md)]",
        "icon-sm": "size-[var(--size-sm)]",
        "icon-lg": "size-[var(--size-lg)]",
      },
    },
    defaultVariants: {
      variant: "default",
