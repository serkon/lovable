...
export default function SettingsPage() {
  ...
  return (
    <div className="bg-background flex min-h-screen flex-col" data-testid="settings-page-container">
      {/* Header */}
      ...
      <main className="mx-auto w-full max-w-2xl space-y-8 p-4" data-testid="settings-main">
        {settingsGroups.map((group, idx) => (
          <div key={idx} className="space-y-3">
            <h4 className="text-muted-foreground px-2 text-xs font-semibold">{group.title}</h4>
            <Card>
              <CardContent className="p-0">
                {group.items.map((item, itemIdx) => (
                  <div key={item.id}>
                    <div
                      className={cn(
                        "hover:bg-muted flex cursor-pointer items-center justify-between p-4 transition-colors",
